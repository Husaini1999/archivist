...
export interface RecommendationSender { sendProposal(projectName: string, proposal: { id: string; title: string; summary: string; evidence: string; expected: string }, token: string): Promise<string | undefined> }
...
export class DailyScheduler {
  ...
  async run(now = new Date()) {
    ...
    for (const project of projects) {
      ...
      try {
        const proposal = await this.orchestrator.suggest(project);
        ...
        const messageId = await this.sender.sendProposal(project.name, proposal, approval.callbackToken);
